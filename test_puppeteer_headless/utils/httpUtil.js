 ...
 */
 ...
 querystring = require('querystring'),
     ...
     *@params opts 基本请求体 opts = {
 	hostname: 'bxjtest.snewfly.com',//必填
 	path: '/jxhd/user/login',//选填 默认/
 	method: 'POST'//选填 默认GET
 		};
     * @params dataObj {'name': "szu",'password': "szu123"}
     */
     var http_request=function(opts,dataObj,onSucc,onErr){
		if(!opts.hostname) return onErr('not set hostname')
     	opts.hostname=opts.hostname.replace('http://','');
